...
Thanks to Proof of Stake, validators and, in fact, everyone in the Ethereum network knows who is gonna submit the next block with a margin of 6 minutes and 24 seconds to 12 minutes and 48 seconds (see the [References](#references) section). That's enough time for bots and validators to do MEV calculations for many of the transactions waiting in the mempool **AND** submit their own with the right amount of gas to make sure their transactions are executed within the next block. Because of this, highly-sensitive operations like swaps between tokens need 1) slippage control and 2) expirarion timestamps so that users will **NOT** incurr in extreme loses due to MEV bots sandwiching/front-running them or validators *"storing"* transactions until they can make a profitable *"chain"*.

This idea can be extended to inter-chain operations pretty easily, so that the requested transactions from one layer to another do not wait idly on the mempool of the first one. However, doing request from Ethereum to zkSync Era through `Mailbox`, the expiration timestamp is set to
...
Some ways to exploit this, from a hacker's POV, is through front-running or sandwich attacks (as transactions requested from Ethereum to zkSync Era are executed sequentially via a priority queue) but there is another way to harm users via rogue validators (see [here](https://news.bitcoin.com/rogue-validator-exploits-mev-bots-on-ethereum-resulting-in-25-3m-in-crypto-losses) for an example, although the affected users where MEV bots). There is another attack path in which a rogue validator builds *"chained transaction-blocks"*. The idea is that, thanks to POS, the odds for a validator to execute sequentially `n > 2` blocks increases with the amount of ETH staked so it is possible to delay a highly profitable transaction and place it between two validator-controlled blocks and sandwich/front-run it on zkSync Era (again, as they are executed sequentially via a priority queue). I'm not gonna deepen more in that, you can go to the [References](#references) section, but know it is possible and pretty dangerous if you are doing, say, an inter-chain [Furocombo](https://furucombo.app/combo) combo worth 3 million dollars.
...
I asked Vlad for the gas costs but he did not know, so I will do the estimations with the Ethereum ones. Just keep in mind that, as the underlying logic for these opcodes are ZK circuits, the gas cost will be higher to cover the computation overhead:
...
- `_refundRecipient` on L2 = `alias(alias(msg.sender))` if `msg.sender` is **NOT** an EOA in L1 **AND** `alias(msg.sender)` is **NOT** an EOA in L1 **and** `_refundRecipient` is `address(0)` when calling `deposit` in one of the bridges.

This is bad as it does imply that the excess of gas may be sent to the wrong `_refundRecipient` or, even worst, if a WETH transfer fails on L2, the whole amount transferred. Although **VERY** unlikely to occur (2^k execution steps to find a collision), I would remove this possibility from happening by modifying the code in both bridges:
...
However, in [AllowList, setDepositLimit](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/contracts/ethereum/contracts/common/AllowList.sol#L129C1-L132C6), such a statement is not enforced:
...
Consider changing [AllowList, setDepositLimit](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/contracts/ethereum/contracts/common/AllowList.sol#L129C1-L132C6) to revert if trying to cap ETH deposits
...
and remove the code after the `require` statement in [Mailbox, _verifyDepositLimit](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/contracts/ethereum/contracts/zksync/facets/Mailbox.sol#L275C1-L281C6)
...
as `Mailbox` is in charge of bridging ETH and the bridges are for ERC20 tokens and WETH.
...
In [bootloader, function getGasPrice](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/bootloader/bootloader.yul#L1030C1-L1052C14), the function checks the correctness of the fee parameters, namely:
...
In [Mailbox, function l2TransactionBaseCost](https://github.com/code-423n4/2023-10-zksync/blob/1fb4649b612fac7b4ee613df6f6b7d921ddd6b0d/code/contracts/ethereum/contracts/zksync/facets/Mailbox.sol#L166C1-L166C36) is not called in the whole project so it seems it will be called by the front-end to show users an stimation of their transaction cost. However, they do permit to pass an arbitrary `_l2GasPerPubdataByteLimit` whilst function `requestL2Transaction` requires that it is equal to `REQUIRED_L2_GAS_PRICE_PER_PUBDATA`, which is `800` (see [here](https://github.com/code-423n4/2023-10-zksync/blob/1fb4649b612fac7b4ee613df6f6b7d921ddd6b0d/code/contracts/SystemConfig.json#L18)). As I do not know a shit about Vue and TS (and your front-end is written in that), I do recommend to hard-code the value in `l2TransactionBaseCost` to **NOT** return misleading estimations to users:
...
[**MsgValueSimulator, line 35**](https://github.com/code-423n4/2023-10-zksync/blob/1fb4649b612fac7b4ee613df6f6b7d921ddd6b0d/code/system-contracts/contracts/MsgValueSimulator.sol#L35)
...
## [L-10] L1 $\rArr$ L2 transactions are not free, but the bootloader behaves like they are
...
[**Mailbox, lines 262 to 272**](https://github.com/code-423n4/2023-10-zksync/blob/1fb4649b612fac7b4ee613df6f6b7d921ddd6b0d/code/contracts/ethereum/contracts/zksync/facets/Mailbox.sol#L262C1-L272C11)
...
In [bootloader, function processL1Tx](https://github.com/code-423n4/2023-10-zksync/blob/c3ff020df5d11fe91209bd99d7fb0ec1272dc387/code/system-contracts/bootloader/bootloader.yul#L878C1-L951C18), the `refundGas` the user will receive is the sum of
...
and it is never used as it assumes that "L1 $\rArr$ L2 transactions are free", which is not true. If the transaction does not revert, then `_refundRecipient` will receive:
...
which equals to $msg.value - (\_l2Value + payToOperator)$. I tried reversing the equation and simplifying it in order to prove that $refundGas * gasPrice \not = msg.value - (\_l2Value + payToOperator)$ but it was a fucking rabbit hole and I started making assumptions that weren't robust enough to be in this POC. I kindly suggest changing the code to:
...
If both equations are equal, that is, $refundGas * gasPrice = msg.value - (\_l2Value + payToOperator)$, then this is more explicit and saves gas by not doing `safeSub`s nor `safeAdd`s (QA). If they are **NOT**, then this code is the correct one, as you are giving the refund recipient exactly the calculated `refundGas`, no more, no less (Medium).
...
The bootloader is provided with $2³² - 1$ gas to execute all submitted transactions in a given batch, so it is expected that all the gas refunds will be below that amount. For L2 transactions it is done in
...
but for L1 $\rArr$ L2 transactions it is not enforced, leading to a possible situation in which the operator refunds, or even pay himself, more than what the bootloader really has. Consider doing in
...
## [L-13] Too many `TODO`s and `unimplemented!` blocks, as well as TBD functionalities

Around **32** TODO sections/comments and **8** unimplemented functionalities. Without them, you cannot test the project as a whole and cover as much edge cases as possible (some of them are even in the `main_vm` folder and affect complex logical branches). Some functionalities are not even developed yet (escape hatch, slashing...). I know this is a work in progress, but doing a contest "halfways" is a bit dangerous.
...
My guess is that `codeLengthInWords` should be equal to the bytes at position `[3]` alone, as the variable's name `bytecodeLenInWords` suggests, that is:
...
so the Low. However, this opens the door for the operator to add an additional log to the queue if there was a system upgrade, so that the first condition reverts, which may be considered as a temporal DOS/griefing. Consider changing
...
as there is no such batch in the current version of zkSync Era.

# Non-critical

## [NC-01] WTF

- [contracts/zksync/contracts/Dependencies.sol](https://github.com/code-423n4/2023-10-zksync/blob/main/code/contracts/zksync/contracts/Dependencies.sol) $\rArr$ not used anywhere.
- [system-contracts/bootloader/bootloader.yul, line 862](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/bootloader/bootloader.yul#L862) $\rArr$ out of place comment?

## [NC-02] Missing comments

- [ContractDeployer, line 307](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/contracts/ContractDeployer.sol#L307C17-L307C45) $\rArr$ what ensures?
- [bootloader, line 1562](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/bootloader/bootloader.yul#L1562C17-L1562C54) $\rArr$ the end of the comment is missing.
- [SystemContractHelper, lines 170 and 173](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/contracts/libraries/SystemContractHelper.sol#L173) $\rArr$ add the [ones being developed by lambdaclass](https://github.com/lambdaclass/zksync_era_precompiles#current-status) to that list.

## [NC-03] Typos

- [SystemContractHelper, line 347](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/contracts/libraries/SystemContractHelper.sol#L347) $\rArr$ `2-bit IS set`, `not 2-bit it set`.
...
- [KnownCodesStorage, lines 16 to 17](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/contracts/KnownCodesStorage.sol#L16C117-L17C10) $\rArr$ repeated `words`.
...
- [SystemContext, line 22](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/contracts/SystemContext.sol#L22C32-L22C48) $\rArr$ `blocks' hashes`, not `blocks's hashes`.
...
- [bootloader, line 38](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/bootloader/bootloader.yul#L38C20-L38C60) $\rArr$ `That means the operator controls ...`, not `That it means that the operator controls ...`.
...
- [bootloader, line 652](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/bootloader/bootloader.yul#L652C57-L652C84) $\rArr$ `... make sure the operator ...`, not `... make sure that the operator`.
...
- [bootloader, line 672](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/bootloader/bootloader.yul#L672C32-L672C35) $\rArr$ remove that `for`.
...
- [bootloader, line 3164](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/bootloader/bootloader.yul#L3164C22-L3164C39) $\rArr$ `Accepts an uint64`, not `Accepts an uint32`.
...
- [bootloader, lines 3170 to 3171](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/bootloader/bootloader.yul#L3170C22-L3171C31) $\rArr$ `Accepts an uint128 and returns wether or not it is a valid uint128`, not `Accepts an uint32 and returns wether or not it is a valid uint64`.
...
- [Governance, line 83](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/contracts/ethereum/contracts/governance/Governance.sol#L83) $\rArr$ `both` is for two elements, not three.
...
- [NonceHolder, line 18](https://github.com/code-423n4/2023-10-zksync/blob/c3ff020df5d11fe91209bd99d7fb0ec1272dc387/code/system-contracts/contracts/NonceHolder.sol#L18C29-L18C31) $\rArr$ it is `mark`, not `either marked` as it is a sentence without a replica.
...
- [NonceHolder, line 29](https://github.com/code-423n4/2023-10-zksync/blob/c3ff020df5d11fe91209bd99d7fb0ec1272dc387/code/system-contracts/contracts/NonceHolder.sol#L29C42-L29C44) $\rArr$ remove the first `at`.
...
- [uma.rs, line 222](https://github.com/code-423n4/2023-10-zksync/blob/c3ff020df5d11fe91209bd99d7fb0ec1272dc387/code/era-zkevm_circuits/src/main_vm/opcodes/uma.rs#L222C12-L222C20) $\rArr$ `Ethereum`, not `Etherium`.
...
- [cycle.rs, line 504](https://github.com/code-423n4/2023-10-zksync/blob/c3ff020df5d11fe91209bd99d7fb0ec1272dc387/code/era-zkevm_circuits/src/main_vm/cycle.rs#L504C8-L504C11) $\rArr$ `Aux`, not `aux`.
...
- In [L1ERC20Bridge, line 162](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/contracts/ethereum/contracts/bridge/L1ERC20Bridge.sol#L162C1-L162C115), the comment says `If the L2 deposit finalization transaction fails, the _refundRecipient will receive the _l2Value`. However,
    ...
    - there is no such a parameter in the function (`l2Value`) and
    - as it is an ERC20 transfer, the call to `Mailbox, requestL2Transaction` made in line 190, has its argument `_l2Value` hard-coded to `0`, so the comment just make no sense. Remove it.

- In [L2ERC20Bridge, line 59](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/contracts/zksync/contracts/bridge/L2ERC20Bridge.sol#L59C1-L59C79), the comment says `@param _l2Receiver The account address that would receive minted ether`. However, the bridge is for ERC20 tokens, not for ETH nor WETH, and failed deposits are claimed on L1, not L2. Remove it.

## [NC-06] Wrong comment in `bootloader, lines 929 to 931`

In [bootloader, lines 929 to 931](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/system-contracts/bootloader/bootloader.yul#L929C1-L931C80), it says `Note, that for now, the L1->L2 transactions are free ...`, which is false as the `isFree` boolean in 

[**Mailbox, line 270**](https://github.com/code-423n4/2023-10-zksync/blob/72f5f16ed4ba94c7689fe38fcb0b7d27d2a3f135/code/contracts/ethereum/contracts/zksync/facets/Mailbox.sol#L270)
...
is hard-coded to `false`, so the gas price is calculated as follows:

```solidity
        {
            params.l2GasPrice = _isFree ? 0 : _deriveL2GasPrice(tx.gasprice, _l2GasPerPubdataByteLimit); // defaults to _deriveL2GasPrice(tx.gasprice, _l2GasPerPubdataByteLimit) as _isFree == false
            uint256 baseCost = params.l2GasPrice * _l2GasLimit;
            require(msg.value >= baseCost + _l2Value, "mv"); // The `msg.value` doesn't cover the transaction cost
        }
```

which is definitely non-zero.
